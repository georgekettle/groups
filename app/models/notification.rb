class Notification < ApplicationRecord
  ...
  after_create_commit {
    if type == 'MessageNotification'
      ...
      # broadcast_prepend_to(recipient, params[:message].channel, :channel_list_item, target: "channel_list_items", partial: "components/lists/channel_list_item", locals: { channel: params[:message].channel, unread_messages: true })
      # broadcast_remove_to(recipient, params[:message].channel, :channel_list_item)
      broadcast_replace_to(recipient, params[:message].channel, :channel_list_item, target: "channel_list_item_#{params[:message].channel.id}", partial: "components/lists/channel_list_item", locals: { channel: params[:message].channel, unread_messages: true, move_to_top: true })
